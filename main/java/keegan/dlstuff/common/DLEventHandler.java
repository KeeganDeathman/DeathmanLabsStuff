...
public class DLEventHandler
{
	...
	public void playerInteract(PlayerInteractEvent event)
	{
		...
		{
			if (placing.get() != null) return;// for mods that do dumb stuff and
												// call this event like MFR
			placing.set(event);
	...
	public static boolean place(EntityPlayer player, World world)
	{
		...
		if (world.isRemote && !player.isSneaking())// attempt to use block
													// activated like normal and
													// tell the server the right
													// stuff
		{
		...
		if (!world.isRemote)
		{
			...
			world.playSoundEffect(pos.x + 0.5, pos.y + 0.5, pos.z + 0.5, part.getBlock().stepSound.func_150496_b(), (part.getBlock().stepSound.getVolume() + 1.0F) / 2.0F, part.getBlock().stepSound.getPitch() * 0.8F);
