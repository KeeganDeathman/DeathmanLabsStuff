...
public class DSCBench extends DSCPart implements IInventory
{
	...
	{
		if(getInstalled().contains(discov.getDependency()))
			{
				if(discov.getIngredients1().isItemEqual(getStackInSlot(0)) && discov.getIngredients1().stackSize <= getStackInSlot(0).stackSize)
				{
					if(discov.getIngredients2().isItemEqual(getStackInSlot(1)) && discov.getIngredients2().stackSize <= getStackInSlot(1).stackSize)
					{
						if(discov.getIngredients3().isItemEqual(getStackInSlot(2)) && discov.getIngredients3().stackSize <= getStackInSlot(2).stackSize)
						{
